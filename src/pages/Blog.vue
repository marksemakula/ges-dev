...
const posts = ref<Post[]>([
  { id: '1', title: 'The Future of Education: Embracing Digital Learning', excerpt: 'Exploring how digital transformation is reshaping the educational landscape and preparing students for tomorrow\'s challenges.', author: 'Dr. Sarah Mukasa', publishedAt: '2024-01-20', category: 'Innovation', tags: ['digital', 'technology'], image: '/images/science.avif', readTime: '5 min read' },
  ...
  { id: '3', title: 'STEM Education Excellence at GES', excerpt: 'Discover how our STEM programmes are preparing students for careers in science, technology, engineering, and mathematics.', author: 'Ms. Grace Nakato', publishedAt: '2024-01-15', category: 'STEM', tags: ['STEM', 'science'], image: '/images/TD.avif', readTime: '6 min read' },
...
.blog-search { display: flex; align-items: center; gap: 0.5rem; background: #fff; border: 1px solid #e5e9f0; border-radius: 8px; padding: 0.5rem 0.875rem; color: #9ca3af; flex: 1; min-width: 200px; max-width: 320px; }
.blog-search__input { border: none; outline: none; font-size: 0.875rem; font-family: inherit; color: #0A2342; background: transparent; width: 100%; }
.blog-cats { display: flex; flex-wrap: wrap; gap: 0.5rem; }
.blog-cat-btn { padding: 0.4rem 0.875rem; border-radius: 20px; border: 1px solid #e5e9f0; font-size: 0.8rem; font-weight: 500; color: #6C757D; background: #fff; cursor: pointer; transition: all 0.15s; font-family: inherit; }
.blog-cat-btn:hover, .blog-cat-btn--active { background: #0A2342; color: #fff; border-color: #0A2342; }

.posts-grid { display: grid; grid-template-columns: 1fr; gap: 1.5rem; }
@media (min-width: 640px) { .posts-grid { grid-template-columns: repeat(2, 1fr); } }
@media (min-width: 1024px) { .posts-grid { grid-template-columns: repeat(3, 1fr); } }

.post-card { border: 1px solid #e5e9f0; border-radius: 12px; overflow: hidden; background: #fff; display: flex; flex-direction: column; transition: box-shadow 0.2s, transform 0.2s; }
.post-card:hover { box-shadow: 0 4px 20px rgba(10,35,66,0.1); transform: translateY(-2px); }
.post-card__img { position: relative; height: 180px; overflow: hidden; background: #f4f7fb; }
.post-card__img img { width: 100%; height: 100%; object-fit: cover; }
.post-card__cat { position: absolute; top: 0.75rem; left: 0.75rem; background: #0A2342; color: #fff; font-size: 0.68rem; font-weight: 600; letter-spacing: 0.08em; text-transform: uppercase; padding: 0.25rem 0.6rem; border-radius: 4px; }
...
.post-card__title { font-size: 0.975rem; font-weight: 700; color: #0A2342; line-height: 1.35; margin-bottom: 0.625rem; }
.post-card__excerpt { font-size: 0.845rem; color: #6C757D; line-height: 1.6; flex: 1; }
.post-card__footer { display: flex; align-items: center; justify-content: space-between; margin-top: 1rem; padding-top: 0.875rem; border-top: 1px solid #f0f2f6; }
.post-card__author { font-size: 0.78rem; font-weight: 600; color: #0A2342; }
.post-read-btn { font-size: 0.78rem; font-weight: 600; color: #00C7B7; background: none; border: none; cursor: pointer; font-family: inherit; transition: color 0.15s; padding: 0; }
.post-read-btn:hover { color: #0A2342; }
